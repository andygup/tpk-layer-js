...
define([
    ...
    function(declare,Extent,query,SpatialReference,TileInfo,TiledMapServiceLayer,TilesStore,TilingScheme,zip,X2JS,autoCenter,Evented){
        return declare("esri.TPKLayer",[TiledMapServiceLayer,Evented],{
            ...
            _parseInMemFiles: function(files,callback){
                ...
                for(var i=0;i < inMemTilesLength;i++){
                    ...
                    if(indexCDI != -1 || indexXML != -1){
                        that._unzipConfFiles(files,i,callback,that);
                    }
                    else{
                        that._unzipTileFiles(files,i,callback,that);
                    }
                }
            },
            ...
            _unzipConfFiles: function(files,token,callback,context){
                files[token].getData(new zip.TextWriter(token),function(data){
                    context._inMemTilesIndex.push("blank");
                    var name = files[data.token].filename.toLocaleUpperCase();
                    context._inMemTilesObject[name]= data.string;
                    var size = context.ObjectSize(context._inMemTilesObject);
                    if(size == context._fileEntriesLength - context._zeroLengthFileCounter - 1){
                        callback(context._inMemTilesObject);
                    }
                });
            },

            _unzipTileFiles: function(files,token,callback,context){
                    ...
                    if(data.size != 0){
                        ...
                        reader.addEventListener("loadend", function (evt) {
                            if(this.token != undefined){
                                ...
                                context._inMemTilesObject[name]= this.result;
                                var size = context.ObjectSize(context._inMemTilesObject);
                                if(size == context._fileEntriesLength - context._zeroLengthFileCounter - 1){
                                    callback(context._inMemTilesObject);
                                }
