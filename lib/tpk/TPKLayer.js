...
define([
    ...
    function(declare,Extent,query,SpatialReference,DataStream,TileInfo,TiledMapServiceLayer,TilesStore,TilingScheme,zip,X2JS,autoCenter,Evented){
        return declare("esri.TPKLayer",[TiledMapServiceLayer,Evented],{
            ...
            throttleMillis: 100,                    // Number of milliseconds to throttle getUrl requests.
            throttleStep: 125,                      // Number of milliseconds to throttle each successive request.
            tileCountThreshold: 15,                 // Minimum no. of tiles in an extent to start throttling
            ...
            constructor:function(/* zip */ tiles){
                ...
                this._parseInMemTiles(tiles,function(array){
...
//                    if(this._autoCenter == null) this._autoCenter = new autoCenter(this.map,this.RECENTER_DELAY);
            ...
            _parseInMemTiles: function(tiles,callback){
                var inMemTilesLength = this._inMemTiles.length;
                if(inMemTilesLength < tiles.length){
                    tiles[inMemTilesLength].getData(new zip.BlobWriter(),function(data){
                        this._inMemTiles.push(data);
                        this._parseInMemTiles(tiles,callback);
                    }.bind(this));
                }
                if(inMemTilesLength == tiles.length){
            ...
            _parseConfCdi: function(tilesInfo,callback){
                ...
                if(m_conf_index != -1){
                    var m_conf_i = this._inBlah[m_conf_index];
//                    m_conf_i.getData(new zip.BlobWriter(),function(data){
                        var reader = new FileReader();
                        reader.onerror = function(event){
                            console.error(new Error( "_parseConfCdi Error: " + event.target.error.code).stack);;
                            that.emit(that.PARSING_ERROR,{msg: "Error parsing conf.cdi file: ", err : event.target.error});
                        }
                        ...
                        reader.readAsArrayBuffer(m_conf_i);
//                    })
                }
            },
            ...
            _parseConfXml:function(initExtent,callback,context) {
                ...
                if (m_conf_config != -1) {
                    var m_conf = this._inBlah[m_conf_config];
//                    m_conf.getData(new zip.BlobWriter(), function (data) {
                        var reader = new FileReader();
                        reader.onerror = function(event){
                            console.error(new Error( "_parseConfXml Error: " + event.target.error.code).stack);;
                            context.emit(this.PARSING_ERROR,{msg: "Error parsing conf.xml file: ", err : event.target.error});
                        }
                        reader.addEventListener("loadend", function (evt) {
                            var result = reader.result;
                            var str = this._bin2String(result);
                        ...
                        reader.readAsArrayBuffer(m_conf);
//                    })
                }
            },
                ...
                this.store.retrieve(url, function(success, offlineTile){
                    if( success )
                    {
                        ...
                        if (bundleIndex != -1 && bundleXIndex != -1) {
                            bundle = this._inBlah[bundleIndex];
                            bundleX = this._inBlah[bundleXIndex];
                        }
                        else {
                        ...
                        if (typeof  bundleX != "undefined") {
                            ...
                            var passthru;
                            tileCount > that.tileCountThreshold ? passthru = false : passthru = true;

                            this._throttler(
                                function(){
                                    that._bundleXReader(bundleX, offset, function (pointer) {
                                        that._bundleReader(bundle, pointer, function (str) {
             ...
             * @private
             */
            _bundleXReader: function(bundleX,offset,callback,context){
//                bundleX.getData(new zip.BlobWriter(),function(data){
                    ...
                    reader.addEventListener("loadend", function(evt) {
                        var pointer = context._getPointer(this.result,offset);
                        callback(pointer);
                    });
...
//                })
             ...
             * @param context
             * @event PROGRESS_EVENT {total : size of payload, loaded : how much has been loaded}
             * @private
             */
            _bundleReader: function(bundle,pointer,callback,context){
//                bundle.getData(new zip.BlobWriter(),function(data){
                    ...
                    reader.addEventListener("progress",function(evt){
                        if(event.lengthComputable){
                            context.emit(context.PROGRESS_EVENT,evt);
                            console.log("Progress total: " + evt.total + ", progress loaded: " + evt.loaded);
                        }
                    });
                    reader.addEventListener("loadend", function(evt) {
                        var str = context._bin2Base64(this.result,pointer);
                        callback(str);
                    });
...
//                })
            ...
            /**
             * This helper function is designed to limit the rate at which your application pulls
             * images out of the various binary Bundle files. If you pan or zoom your application
             * that can fire off many getUrl requests that really slow down or max out the browser.
             *
             * Once the images are written to the database then performance should be very fast the
             * next time the user pans or zooms to that area.
             *
             * Set the step property in millis to slow down the getUrl request rate. It will start
             * incrementing using the throttleMillis property as the initial value.
             *
             * Slower devices may need more throttling when there are many tiles within an extent.
             * To set this property, zoom down as close to the earth as possible and monitor
             * the tile generating performance. You'll have to experiment to find the best possible
             * settings for your various tile packages.
             * @param func The function you wish to use.
             * @param passthru Execute the function immediately.
             * @param step The number of millis to wait each successive request.
             * @private
             */
            _throttler: function(func,passthru,step) {
                this._timerStarted == true ? this.throttleMillis += step : this.throttleMillis = step;
