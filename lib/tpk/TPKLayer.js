define([
    ...
    "tpk/zip","tpk/xml2json"],
    function(declare,Extent,query,SpatialReference,DataStream,TileInfo,TiledMapServiceLayer,TilesStore,TilingScheme,zip,X2JS){
        return declare("test",TiledMapServiceLayer,{
            ...
            constructor:function(/* zip */ tiles){
                ...
                this._parseConfCdi(tiles,function(initExtent,result){
                    ...
                    var minZoom = this.tileInfo.lods[0];
                    var maxZoom = this.tileInfo.lods[this.tileInfo.lods.length - 1];
                    this._prepareForOffline(minZoom.level,maxZoom.level,this.initialExtent);
            ...
            _validate: function(){
                ...
                if( /*false &&*/ this._store.isSupported() )
                {
                    this._store.init(function(result){
                        if(result == false){
                            console.error(new Error( "There was an error initializing the database.").stack);
                        }
                        else{
                            this._store.usedSpace(function(size,err){
             ...
             */
            _prepareForOffline: function(minLevel, maxLevel, extent, reportProgress)
            {
                ...
                this._parseTiles(0, cells, reportProgress);
            },

            _parseTiles: function(i, cells, reportProgress)
            {
                var cell = cells[i];

                this._storeTile(url,layersDir,cell.level,cell.row,cell.col, function(success, error)
                {
                    if(!success)
                    {
                    ...
                    {
                        this._parseTiles(i+1, cells, reportProgress);
                    }

                }.bind(this));
            },

            _storeTile: function(url,layersDir,level,row,col,callback){
            ...
            _getInMemTiles: function(url,layersDir,level,row,col,callback){
                ...
                this._store.retrieve(url, function(success, offlineTile){
                    if( success )
                    {
