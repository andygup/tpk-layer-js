        ...
        return declare("esri.TPKLayer",[TiledMapServiceLayer,Evented],{
            ...
            extend: function(files){
                ...
                this._parseInMemFiles(files,function (buffer){
                    ...
                    this._parseConfCdi(buffer,function(initExtent,result){
                        ...
                        this.__worker.postMessage({type:"tiles",array:this._inMemTilesObject});
                    }.bind(this._self));
                }.bind(this._self));
            },
            ...
            getTileUrl:function(level,row,col){
                ...
                if(this._inMemTilesObject != {}) {
                    ...
                    this._getInMemTiles(url,layersDir, level, row, col, count,tileid,function (result,tileid,url) {
            ...
            _parseInMemFiles: function(files,callback){
                var that = this;
                var inMemTilesLength = this._inMemTilesIndex.length;
                if(inMemTilesLength < this._fileEntriesLength){
                    ...
                    if(indexCDI != -1 || indexXML != -1){
                        files[0].getData(new zip.TextWriter(),function(data){
                            ...
                            that._inMemTilesObject[name]= data;
                            files.shift();
            ...
            _getInMemTiles: function(url,layersDir,level,row,col,tileCount,tileId,callback){
                ...
                var that = this._self;
                var url = url; console.log("TEST URL " + url)
                var db = this.store;
                ...
                this.store.retrieve(url, function(success, offlineTile){
                    if( success )
                    {
                        ...
                        if(typeof(Worker) !== "undefined"){
                            this.__worker.postMessage({type:"image",imageIndex:bundleIndex,pointer:pointer,tileid:tileId,url:url})
                            this.__worker.onmessage = function(evt){
                                switch(evt.data.type){
                                    case "image":
                                        if (that._isDBWriteable){
                                            console.log("Writing to database: " + evt.data.url)
                                            that._storeTile(evt.data.url, evt.data.image, db);
                                        }
                                        callback(evt.data.image,evt.data.tileid,evt.data.url);
                                        break;
                                }
                            }
                        }
