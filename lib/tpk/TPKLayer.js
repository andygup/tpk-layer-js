...
define([
    ...
    "tpk/zip","tpk/xml2json","dojo/Evented"],
    function(declare,Extent,query,SpatialReference,DataStream,TileInfo,TiledMapServiceLayer,TilesStore,TilingScheme,zip,X2JS,Evented){
        return declare("esri.TPKLayer",[TiledMapServiceLayer,Evented],{
            ...
            _validate: function(){
                ...
                if(!window.File && !window.FileReader && !window.Blob && !window.btoa && !window.DataView){
                    ...
                    this.emit(this.VALIDATION_ERROR,{msg:"TPKLayer library is not supported"});
                }
                ...
                if( /*false &&*/ this.store.isSupported() )
                {
                    this.store.init(function(result){
                        if(result == false){
                            ...
                            this.emit(this.DATABASE_ERROR,{msg:"Unable to initialize the database."});
                        }
                        else{
                            this.store.usedSpace(function(size,err){
                                ...
                                if(mb > this.MAX_DB_SIZE){
                                    ...
                                    this.emit(this.DATABASE_ERROR,{msg:"Database full! " + err});
                                }
                ...
                else
                {
                    ...
                    this.emit(this.VALIDATION_ERROR,{msg:"IndexedDB is not supported."});
                }
            },
                ...
                if(m_conf_index != -1){
                    ...
                    m_conf_i.getData(new zip.BlobWriter(),function(data){
                        ...
                        reader.onerror = function(event){
                            ...
                            this.emit(this.PARSING_ERROR,{msg:event.target.error});
                        }
                ...
                bundle.getData(new zip.BlobWriter(),function(data){
                    ...
                    reader.addEventListener("progress",function(evt){
                        if(event.lengthComputable){
                            this.emit(this.PROGRESS_EVENT,{msg:evt});
            ...
            _storeTile: function(url,base64Str,db){
                ...
                db.store(tile,function(success,err){
                    if(err){
                        ...
                        this.emit(this.DATABASE_ERROR,{msg:"Error writing to database. " + err});
                    }
                });
            },
