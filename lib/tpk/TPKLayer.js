        ...
        return declare("esri.TPKLayer",[TiledMapServiceLayer,Evented],{
            ...
            _parseInMemFiles: function(files,callback){
                ...
                if(inMemTilesLength < this._fileEntriesLength){
                    ...
                    var index = name.indexOf("CONF.CDI",0);
                    if(index != -1){
            ...
            _parseConfXml:function(initExtent,callback) {
                ...
                if (m_conf_config != -1) {
                    ...
                    var result = m_conf;
                    var str = this._bin2String(result,0);
                ...
                this.store.retrieve(url, function(success, offlineTile){
                    if( success )
                    {
                        ...
                        if (bundleIndex == -1 || bundleXIndex == -1) {
                            ...
                            var str = that._bin2Base64(this._inMemTiles[bundleIndex],pointer,function(result){
             ...
             * @private
             */
            _bin2Base64: function(/* ArrayBuffer */arrayBuffer,/* int */ position,callback){
//                console.log("START " + new Date().getMilliseconds() + ", " + position)
...
//                console.log("DONE " + new Date().getMilliseconds() + ", " + position)
//                callback (btoa(string));
console.log("time " + new Date().getMilliseconds() + ", " + position)
