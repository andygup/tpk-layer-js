    ...
    function(declare,Extent,query,SpatialReference,TileInfo,TiledMapServiceLayer,TilesStore,TilingScheme,zip,X2JS,autoCenter,Evented){
        return declare("esri.TPKLayer",[TiledMapServiceLayer,Evented],{
            ...
            extend: function(files){
                ...
                this.__worker = new Worker(this.LIBRARY_PATH + 'imageWorker.js');
                ...
                this._parseInMemFiles(files,function (buffer){
                    ...
                    this._parseConfCdi(buffer,function(initExtent,result){
                        ...
                        this.__worker.postMessage({type:"tiles",array:this._inMemTiles2});
                    }.bind(this._self));
                }.bind(this._self));
            },
            ...
            getTileUrl:function(level,row,col){
                ...
                if(this._inMemTiles2 != {}) {
                    ...
                    this._getInMemTiles(url,layersDir, level, row, col, count,tileid,function (result,tileid) {
            ...
            _parseInMemFiles: function(files,callback){
                ...
                if(inMemTilesLength < this._fileEntriesLength){
                    ...
                    var name = files[0].filename.toLocaleUpperCase();
                    ...
                    if(indexCDI != -1 || indexXML != -1){
                        files[0].getData(new zip.BlobWriter(),function(data){
                            ...
                            reader.addEventListener("loadend", function (evt) {
                                ...
                                if(index != -1){
                                    that.TILE_PATH = name.slice(0,index);
                                }
                                that._inMemTiles2[name]= this.result;
                                files.shift();
                                that._parseInMemFiles(files,callback);
                            });
                            reader.readAsBinaryString(data); //open bundleX
                        }.bind(that));
            ...
            _parseConfCdi: function(tilesInfo,callback){
                ...
                var m_conf_i = this._inMemTiles2[this.TILE_PATH + "CONF.CDI"];
             ...
             * @private
             */
            _parseConfXml:function(initExtent,callback) {
                var m_conf = this._inMemTiles2[this.TILE_PATH + "CONF.XML"];
             ...
             * @private
             */
            _getInMemTiles: function(url,layersDir,level,row,col,tileCount,tileId,callback){
                ...
                this.store.retrieve(url, function(success, offlineTile){
                    if( success )
                    {
                        ...
                        var pointer = that._getPointer(this._inMemTiles2[path + ".BUNDLX"], offset);
                        if(typeof(Worker) !== "undefined"){
                            this.__worker.postMessage({type:"image",imageIndex:bundleIndex,pointer:pointer,tileid:tileId})
                            this.__worker.onmessage = function(evt){
                                switch(evt.data.type){
                                    case "image":
                                        if (that._isDBWriteable)that._storeTile(url, evt.data.image, db);
                                        callback(evt.data.image,evt.data.tileid);
                                        break;
                                }
                            }
                        }
                        else{
                            //If workers aren't supported then take the performance hit and
                            //run image conversion on the main thread.
                            that._buffer2Base64(this._inMemTiles2[bundleIndex],pointer,function(result){
