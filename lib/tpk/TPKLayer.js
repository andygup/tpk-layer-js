        ...
        return declare("esri.TPKLayer",[TiledMapServiceLayer,Evented],{
            ...
            _parseInMemFiles: function(files,callback){
                ...
                for(var i=0;i < inMemTilesLength;i++){
                    ...
                    if(files[i].compressedSize == 0) that._zeroLengthFileCounter++;

                    var indexCDI = name.indexOf("CONF.CDI",0);
                    var indexXML = name.indexOf("CONF.XML",0);
                    if(indexCDI != -1 || indexXML != -1){
                        files[i].getData(new zip.TextWriter(null,i),function(data){
                            that._inMemTilesIndex.push("blank");
                            var name = files[data.count].filename.toLocaleUpperCase();
                            that._inMemTilesObject[name]= data.string;
                        ...
                        files[i].getData(new zip.BlobWriter(i),function(data){
                            var reader = new FileReader();
                            reader.count = data.count;
                            ...
                            reader.addEventListener("loadend", function (evt) {
                                if(this.count != undefined){
                                    that._inMemTilesIndex.push("blank");
                                    var name = files[this.count].filename.toLocaleUpperCase();
